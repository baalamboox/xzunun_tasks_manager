$(document).ready(() => {
    ...
    $('#boton_siguiente').click(() => {
        if($('#usuario').val() != '') {
            $.ajax({
                type: 'POST',
                url: 'control/administrador/inicio-sesion.php',
                data: {
                    'usuario': $('#usuario').val()
                },
                success: resultado => {
                    if(resultado != 0) {
                        if(JSON.parse(resultado).email_institucional == $('#usuario').val()) {
                            ...
                            $('#boton_ingresar').click(() => {
                                ...
                                if(contrasenia != '') {
                                    if(JSON.parse(resultado).contrasenia == contrasenia) {
                                        window.location.href = 'view/administrador/plantilla-barra-lateral.php';
                                    } else {
                                        Swal.fire({
                                            customClass: {
                                                title: 'titulo',
